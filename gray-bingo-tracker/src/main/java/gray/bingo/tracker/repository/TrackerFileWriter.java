...
public class TrackerFileWriter {
    ...
    public TrackerFileWriter(long singleFileSize, int totalFileCount, String baseDir, String appName, boolean usePid) {
        ...
        log.info("[      BINGO_TRACKERS] >>> 创建 FileWriter,单文件大小[ {}MB ], 最大文件数[ {} ].", (singleFileSize / 8 / 1024 / 1024), totalFileCount);
        ...
        try {
            ...
            log.info("[      BINGO_TRACKERS] >>> 创建 FileWriter 失败: {}.", e);
    ...
    private void removeMoreFiles() throws Exception {
        ...
        for (int i = 0; i < list.size() - totalFileCount + 1; i++) {
            ...
            log.info("[      BINGO_TRACKERS] >>>  Removing metric file: " + fileName);
            new File(indexFile).delete();
            log.info("[      BINGO_TRACKERS] >>>  Removing metric index file: " + indexFile);
        }
    }
