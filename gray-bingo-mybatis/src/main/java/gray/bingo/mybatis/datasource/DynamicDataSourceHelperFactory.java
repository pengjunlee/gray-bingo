 ...
 */
@Slf4j
public class DynamicDataSourceHelperFactory extends BingoHelperFactory {
    ...
    public void build(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
        if (BingoMeta.helperEnabled(BingoCst.BINGO_HELPER_DYNAMIC_DB)) {
            ...
            if (ObjectUtils.isEmpty(dbMap)) {
                log.warn("[     HELPER_FACTORY]  -- 注册组件 [ {} ] 失败，原因自定义数据源为空，请检查配置项: [ {} ] ！", helperName(), "bingo.db");
                return;
            }
            //  初始化默认配置
            ...
            log.warn("[     HELPER_FACTORY]  -- 注册组件 [ {} ] 失败，原因组件未启用，请检查配置项: [ {} ] ！", helperName(), BingoCst.CONF_HELPER_ENABLES);
        }
    }
    ...
    private Integer getLongValue(Map<String, String> myMap, String key) {
        String longValue = myMap.getOrDefault(key, defaultMap.get(key));
